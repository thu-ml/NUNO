...
from .interp_unet import UNet3d
...
def main(train_a_sd, train_u_sd, test_a_sd, test_u_sd):
    ...
    model = UNet3d(
        n_channels=output_dim*n_subdomains, 
        n_classes=output_dim*n_subdomains).cuda()
    ...
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            optimizer.zero_grad()
            out = model(x)[..., :-1, :].reshape(batch_size, s1_padded, s2_padded, 
...
if __name__ == "__main__":
    ...
    # Increase by 1
    train_a_sd = torch.concat((train_a_sd, 
        train_a_sd[..., 0:1, :]), dim=-2)
    test_a_sd = torch.concat((test_a_sd, 
        test_a_sd[..., 0:1, :]), dim=-2)
